...
Form1->Frm=(double)1/StrToInt(Edit1->Text);
 Form1->FM=StrToInt(Edit1->Text);
Form1->Path=Edit2->Text;
ShowMessage("Se ha creado un proyecto nuevo ubicado en: "+ Form1->Path+" con período de muestreo para la Serie principal de "  +Form1->Frm+ " segundos.");
Form1->Path_Name=ExtractFileName(Form1->Path);
Form1->Outline1->Lines->Add(Form1->Path_Name);
Form1->FFT_L();
}
